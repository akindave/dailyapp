...
const morningPrayerImage = require('../assets/img/sunrise.jpg');
...
const renderItem = ({item})=>{
  return(
    <View >
      {
        item.id == 1 ? <IntroPage item={item} title="Night"/> : ''
      }
...
export default function NightPrayerScreen({navigation}) {
  ...
  const fetchData = async () => {
    try {
    const resp = await fetch("https://daotechnology.onrender.com/api/v1/get/all/evening/prayer/evening_prayer");
